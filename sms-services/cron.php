...
# Parse the command line
    ...
    foreach($argv as $arg)
    {
        ...
        switch($param)
        {
            ...
            case '--log-file':
                ...
                {
                    $file_handler = fopen($value, 'w') or die("can't open file");
                    fclose($file_handler);
                }
    ...
    if (!isset($_SERVER['ENVIRONMENT'])) {
        define('CRON_CI_INDEX', '/var/www/html/production/sms-services/index.php'); // Production server index.php file
    } else {
        define('CRON_CI_INDEX', '/var/www/html/'.$_SERVER['ENVIRONMENT'].'/sms-services/index.php');   // Your CodeIgniter main index.php file
    }      
