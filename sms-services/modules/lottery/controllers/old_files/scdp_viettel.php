...
class SCDP_viettel extends MY_Controller {
        
    private $sconfig;  
          
    private $lottery_catalog = array();
    private $lottery_code = NULL;    
    private $open_days;        
    private $region;
    ...
    public function __parse_new_mo()
    {
        if ($this->MO->argument == '')
        {
            write_log('error',"Empty lottery code");
            $this->scenario = "sc_missing_lottery_code";
            return;
        }         
    ...
    {                       
        if ($this->MO->argument == '' || $this->trigger == 'new_mo')
        {
            return;
        } 
                
        //Reload lottery code and its relevant info
        $this->lottery_code = $this->MO->argument;                  
        if ( ! $this->region)
        {
            $this->region = $this->Lottery_model->get_region($this->lottery_code);                        
        }                                   
    ...
    {                
        ...
        {
            case 'prediction':                    
                $data['predict'] = $this->Lottery_model->predict($this->lottery_code,NULL,$this->MO->msisdn,12,$this->MO->date);
                $data['predict'] = array_slice($data['predict'],6,3);
                $data['predict_DB'] = $this->Lottery_model->predict($this->lottery_code,'DB',$this->MO->msisdn,10,$this->MO->date);                
                $data['date'] = date('d/m',strtotime($this->MO->date));
                $data['region'] = $this->Lottery_model->get_region($this->lottery_code);
                break;
    ...
    protected function sc_missing_lottery_code()
    {
        $this->reply_string("Ban nhap thieu ma xo so can xem, xin vui long kiem tra lai tin nhan");
    }
    ...
    {    
        $this->sc_7027();              
        $this->MO->balance = $this->number_of_mt[$this->MO->balance]-1;
    }       
    ...
    {           
        $this->MO->balance = $this->number_of_mt[$this->MO->balance];
