    ...
    protected function active_cois()
    {
        ...
        {            
            ...
            {                   
                ...
                {
                    $this->Evr->not_found[] = $coi['secret_number'];       
                    write_log('debug',"[WARNING] COI ".$coi['secret_number']." in status of 'created'. Change it to 'inactive' first!");             
                }
                elseif ($this->Coi_model->status != 'inactive')
                {
                    $this->Evr->activated[] = $this->Coi_model->secret_number;
                    write_log('debug',"[WARNING] COI ".$coi['secret_number']." in status of '".$this->Coi_model->status."'. Cannot active!");
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        write_log('info',"Miss OID parameter to active COI ".$this->Coi_model->serial_number.".");
                    }
                }
            }
        }
    ...
    public function cancel_cois()
    {
        foreach($this->MO->args as $coi)
        {
            ...
            {   
                ...
                {
                    $this->Evr->not_found[] = $coi;         
                    write_log('debug',"[WARNING] COI ".$coi." in status of 'created'. Change it to 'inactive' first!");           
                }
                elseif ($this->Coi_model->status == 'cancelled')
                {
                    $this->Evr->cancelled[] = $this->Coi_model->serial_number;
                    write_log('info',"COI ".$coi." in status of '".$this->Coi_model->status."'. Cannot cancel!");
                }
                else
                {
                    //Cancel the COI
        ...
        foreach($this->MO->args as $coi)
        {
            ...
            {   
                ...
                elseif ($this->Coi_model->status == 'lost')
                {
                    ...
                    write_log('debug',"[WARNING] COI ".$coi." in status of '".$this->Coi_model->status."'. Cannot mark as lost!");
                }
                else
                {
